...
// Guessing these vars were here in v1.3, rather than
// further below (for correct ordering of vars in the EXE).
// Also defining them as non-static in v1.3, for
// consistency with the surrounding vars.
#if (APPVER_ROTTREV < AV_RR_RTSRC)
objtype           *SNAKEHEAD,*SNAKEEND,*PARTICLE_GENERATOR,*EXPLOSIONS;
#if (SHAREWARE == 0)
...
// *** VERSIONS RESTORATION ***
// Guessing these vars were defined above in v1.3
// (for correct ordering of vars in the EXE), and also as non-static vars.
#if (APPVER_ROTTREV >= AV_RR_RTSRC)
...
//MED
      ...
      // *** VERSIONS RESTORATION ***
      // Store speed based on version.
      // Further uncomment the two lines that follow it, based on version (keep the third one commented out)
#if (APPVER_ROTTREV < AV_RR_RTSRC)
      new->speed = nspeed;
#else
      new->speed = nspeed>>1;
