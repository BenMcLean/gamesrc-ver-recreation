...
void ReadROTTMap
   ...
   if ( !RTLMap.used )
      ...
      if ( RTLMap.RLEWtag == REGISTERED_TAG )
         {
#if (APPVER_ROTTREV == AV_RR_RTLD13A)
         Error( "Can't use maps from the registered game in Special   version." );
#else
      ...
      if ( RTLMap.RLEWtag != SHAREWARE_TAG )
         {
#if (APPVER_ROTTREV == AV_RR_RTLD13A)
         Error( "Can't use modified maps in Special   version." );
#else
...
// pheight=maxheight-32;
   CountAreaTiles();
	...
	if (gamestate.SpawnEluder || gamestate.SpawnDeluder)
      {
      // *** VERSIONS RESTORATION ***
      // Disable the loop and call function just once
//MED
