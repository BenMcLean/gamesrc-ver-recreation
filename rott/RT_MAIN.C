...
// HACK for ordering variables as in the Site License v1.3 EXE...
#if (APPVER_ROTTREV < AV_RR_RTSRC)
#define gametime lasttics // Actually an unused var in the same EXE (and later)
#define infopause prompt
#define turbo fast
#endif
...
// *** VERSIONS RESTORATION ***
#if (APPVER_ROTTREV >= AV_RR_RTLD13B)
boolean quiet = false;
#endif
...
//      }
   if (standalone==false)
      ...
      if ( !NoSound && !IS8250 )
         {
         // *** VERSIONS RESTORATION ***
         // No check of "quiet" here and later
#if (APPVER_ROTTREV >= AV_RR_RTLD13B)
         if (!quiet)
#endif
            printf( "MU_Startup: " );
...
void CheckCommandLineParameters( void )
{
   ...
   // *** VERSIONS RESTORATION ***
#if (APPVER_ROTTREV < AV_RR_RTLD13B)
                       "TIMELIMIT","MAXTIMELIMIT","NOECHO","DEMOEXIT",NULL};
#else
   ...
   noecho = false;
   // *** VERSIONS RESTORATION ***
   // Variable didn't exist. Also disabling later accesses to it.
#if (APPVER_ROTTREV >= AV_RR_RTLD13B)
   quiet = false;
#endif
