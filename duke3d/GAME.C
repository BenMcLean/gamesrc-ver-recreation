...
#else
#define IDFSIZE 479985668
#define IDFILENAME "WW2.IDF"
#endif
#else
#define IDFSIZE 479985668
#define IDFILENAME "NAM.IDF"
#endif
#else
#define IDFSIZE 479985668
    ...
    while ((packbufleng = getpacket(&other,packbuf)) > 0)
    {
        switch(packbuf[0])
        {
            ...
            case 6:
				...
				{
#ifdef NAM
					sprintf(namtemp,"\nYou cannot play %s with different versions.",basename);
				gameexit(namtemp);
#else
...
#ifdef WW2		  
          printext256(4L,138L,31,0,"Read ww2help.txt for information.",0);
#else
...
#ifdef INVNAM
			u=0;
			if( i>0 ) //(i == STEROIDS_ICON ) || ( i == JETPACK_ICON) )
				u=1;
			else
				// show only if NOT steroids or jetpack
#endif
...
// don't display in WW2
    ...
    if (u&2)
    {
		...
		{
			digitalnumber(64,200-17,lAmount,-16,10+16+128);
		}
#else
        if (u != 0xffffffff) patchstatusbar(52,183,75,193);
        digitalnumber(64,200-17,p->shield_amount,-16,10+16+128);
...
#ifdef INVNAM
				if( ( i== STEROIDS_ICON) || (i==JETPACK_ICON) )
					;	// nothing
				{
#endif					
...
#ifdef INVNAM
				if( (p->inven_icon == 2 /* STEROIDS */) ||
					(p->inven_icon == 4 /* JETPACK */)
					)
					; // nothing
				else
#endif
...
void showtwoscreens(void)
{
    ...
    nextpage(); for(i=63;i>0;i-=7) palto(0,0,0,i);
    totalclock = 0;
//    while( !KB_KeyWaiting() && totalclock < 00) getpackets();
...
fclose(fp);
                    ...
                    case 1:
...
#ifdef WW2
						SetGameVarID(g_iReturnVarID, 400, -1, myconnectindex);
						OnEvent(EVENT_CHEATGETSTEROIDS, -1, myconnectindex, -1);
						if( GetGameVarID(g_iReturnVarID, -1, myconnectindex) >=0)
						{
							ps[myconnectindex].steroids_amount =
								GetGameVarID(g_iReturnVarID, -1, myconnectindex);
						}
						
						SetGameVarID(g_iReturnVarID, 1200, -1, myconnectindex);
						OnEvent(EVENT_CHEATGETHEAT, -1, myconnectindex, -1);
						if( GetGameVarID(g_iReturnVarID, -1, myconnectindex) >=0)
						{
						...
						SetGameVarID(g_iReturnVarID, 200, -1, myconnectindex);
						OnEvent(EVENT_CHEATGETBOOT, -1, myconnectindex, -1);
						if( GetGameVarID(g_iReturnVarID, -1, myconnectindex) >=0)
						{
						...
						SetGameVarID(g_iReturnVarID, 100, -1, myconnectindex);
						OnEvent(EVENT_CHEATGETSHIELD, -1, myconnectindex, -1);
						if( GetGameVarID(g_iReturnVarID, -1, myconnectindex) >=0)
						{
						...
						SetGameVarID(g_iReturnVarID, 6400, -1, myconnectindex);
						OnEvent(EVENT_CHEATGETSCUBA, -1, myconnectindex, -1);
						if( GetGameVarID(g_iReturnVarID, -1, myconnectindex) >=0)
						{
						...
						SetGameVarID(g_iReturnVarID, 2400, -1, myconnectindex);
						OnEvent(EVENT_CHEATGETHOLODUKE, -1, myconnectindex, -1);
						if( GetGameVarID(g_iReturnVarID, -1, myconnectindex) >=0)
						{
						...
						SetGameVarID(g_iReturnVarID, 1600, -1, myconnectindex);
						OnEvent(EVENT_CHEATGETJETPACK, -1, myconnectindex, -1);
						if( GetGameVarID(g_iReturnVarID, -1, myconnectindex) >=0)
						{
						...
						SetGameVarID(g_iReturnVarID, max_player_health, -1, myconnectindex);
						OnEvent(EVENT_CHEATGETFIRSTAID, -1, myconnectindex, -1);
						if( GetGameVarID(g_iReturnVarID, -1, myconnectindex) >=0)
						{
...
#ifdef WW2
#ifdef EDUKE
        if( KB_KeyPressed(sc_M) )
#else
        if( KB_KeyPressed(sc_W) )
#endif
#else
...
#else
    puts(" /t#           Respawn, 1 = Enemies, 2 = Items, 3 = Inventory, x = All");
    puts(" /c#           MP mode, 1 = GIMatch(spawn), 2 = Coop, 3 = GIMatch(no spawn)");
#endif
#else
    puts(" /t#           Respawn, 1 = Enemies, 2 = Items, 3 = Inventory, x = All");
    puts(" /c#           MP mode, 1 = GruntMatch(spawn), 2 = Coop, 3 = Gruntmatch(no spawn)");
#endif
#else
    puts(" /t#           Respawn, 1 = Monsters, 2 = Items, 3 = Inventory, x = All");
    puts(" /c#           MP mode, 1 = DukeMatch(spawn), 2 = Coop, 3 = Dukematch(no spawn)");
...
#else
    puts(" /u#########   User's favorite weapon order (default: 2438560791)");
#endif
#else
    puts(" /u#########   User's favorite weapon order (default: 3425689071)");
#endif
...
#else
                                puts("GI Match (no spawn).");
#endif
#else
                                puts("Gruntmatch (no spawn).");
#endif
#else
                                puts("Dukematch (no spawn).");
#endif
...
char testcd( char *fn )
{
 ...
 struct _SREGS sr;
#ifdef NAM
 // don't force drive to be CDROM
#ifdef WW2GI

 // DO force drive to be CDROM
 if( IDFSIZE != 9961476 )
 {
     drive = toupper(*fn)-'A';
...
char namdrive[_MAX_DRIVE];
char namdir[_MAX_DIR];
char basename[_MAX_FNAME];
char baseext[_MAX_EXT];
char namtemp[_MAX_PATH];
...
void main(int argc,char **argv)
{
    ...
    printstr(0,0,"                                                                                ",79);
#ifdef NAM
	_splitpath(argv[0],namdrive,namdir,basename,baseext);
#ifdef EDUKE
	// force base name to 'duke3d'
	strcpy(basename,"DUKE3D");
#endif
	sprintf(nam_head2,"%s  %s",basename,"  Full Version v"VERSION);
#endif
...
#ifdef NAM
#ifdef WW2
    printstr(0,1,"       Copyright (c) 1999 GT Interactive. (c) 1996 3D Realms Entertainment      ",79);
#else
    printstr(0,1,"       Copyright (c) 1998 GT Interactive. (c) 1996 3D Realms Entertainment      ",79);
#endif
#else
...
#ifdef NAM
#ifdef EDUKE
//	sprintf(namtemp,"Prerelease version.  Do not distribute\n");
//	puts(namtemp);
...
//	puts("\n");
	...
	puts(namtemp);
	sprintf(namtemp,"Copyright (C) 2000 Matt Saettler.  All Rights Reserved.\n");
	puts(namtemp);
#else
#ifdef WW2	
	sprintf(namtemp,"Additional engine modifications by Matt Saettler.\n");
...
//#ifdef AUSTRALIA
...
#ifdef NAM
	sprintf(namtemp,"%s.grp",basename);
    initgroupfile(namtemp);
#else
    initgroupfile("duke3d.grp");
#endif
#ifdef EDUKE
	strcpy(confilename,"EDUKE.CON");
	if(SafeFileExists(confilename) == 0)
	{
		printf("Could not find Enhanced Duke Startup CON file '%s'.\n",confilename );
		printf("Resetting to Duke Default\n");
		strcpy(confilename,"GAME.CON");
	}
	else
	{
		printf("Using Enhanced Duke Startup CON File: '%s'\n",confilename);
	}
#endif
    ...
    if(memorycheckoveride == 0)
    {
        ...
        {
#ifdef NAM			
            printf("You don't have enough free memory to run %s.\n",basename);
            puts("The DOS \"mem\" command should report 6,800K (or 6.8 megs)");
            puts("of \"total memory free\".\n");
            printf("%s requires %ld more bytes to run.\n",basename,3162000-350000-totalmemory);
#else
    ...
    {
		// player specified a map name on command line
        ud.m_level_number = 7;
