#ifdef WW2
#define WW2WEAPON
#define WW2WEPDISPLAYPISTOL

#ifndef EDUKE
// WW2 but not EDUKE
#define WW2WEPDISPLAY
#endif
#endif
...
//                sa = getangle(ps[j].oposx-sx,ps[j].oposy-sy);
        ...
        case HANDHOLDINGLASER:
            ...
            if(j == 1)
            {
				...
				{
					long lLifetime=GetGameVar("STICKYBOMB_LIFETIME", NAM_GRENADE_LIFETIME, -1, p);
					long lLifetimeVar=GetGameVar("STICKYBOMB_LIFETIME_VAR", NAM_GRENADE_LIFETIME_VAR, -1, p);
...
//AddLog("DoFire()");	
	...
	{
//sprintf(g_szBuf,"%d: using Ammo. was: %d",p->curr_weapon,p->ammo_amount[p->curr_weapon]);
// AddLog(g_szBuf);
	...
	{
//sprintf(g_szBuf,"DoFire():firesound(%ld)",aplWeaponFireSound[p->curr_weapon][snum]);
//AddLog(g_szBuf);
...
//sprintf(g_szBuf,"DoFire():shoot(%ld)",aplWeaponShoots[p->curr_weapon][snum]);	
//AddLog(g_szBuf);
...
//sprintf(g_szBuf,"%d: Do fire: exit ammo: %d",p->curr_weapon,p->ammo_amount[p->curr_weapon]);
//AddLog(g_szBuf);
}
    ...
    else switch(cw)
    {
        case KNEE_WEAPON:
            if( (*kb) > 0 )
            {
                ...
                if( (*kb) < 5 || (*kb) > 9 )
				{
//sprintf(g_szBuf,"KNEE:A %ld %ld",(long)weapon_xoffset+220-(p->look_ang>>1),
//		(long)looking_arc+250-gun_pos);
//AddLog(g_szBuf);
				...
				{
//sprintf(g_szBuf,"KNEE:B %ld %ld",(long)weapon_xoffset+160-(p->look_ang>>1),
//		(long)looking_arc+214-gun_pos);
//AddLog(g_szBuf);
        ...
        case RPG_WEAPON:
            ...
            {
#ifdef WW2WEPDISPLAY
                if(*kb < aplWeaponTotalTime[RPG_WEAPON][snum])
#else				
                if(*kb < 8)
#endif
                {
				...
				{
					// down 
                    gun_pos -= 10*((*kb)-aplWeaponTotalTime[p->curr_weapon][snum]); //D
//					weapon_xoffset+=80*(*kb-aplWeaponTotalTime[cw][snum]);
				...
				else
				{
					...
					// up and left
                    gun_pos -= 10*(aplWeaponReload[p->curr_weapon][snum]- (*kb)); //U
//					weapon_xoffset+=80*(*kb-aplWeaponTotalTime[cw][snum]);
				}
#endif
            }
...
//						CHAINGUN,gs,o,pal);
			...
			{
                 myospal(weapon_xoffset+188-(p->look_ang>>1),looking_arc+243-gun_pos,
                    CHAINGUN+2,gs,o,pal);
			}
			// else we are in 'reload time'
			...
			else
			{
					   ...
					   )
				{
					// second segment
					// down 
                    gun_pos += 80; //5*(iFifths(*kb)-aplWeaponTotalTime[p->curr_weapon][snum]); //D
					   ...
					   )
				{
					// fourth segment
					// down 
                    gun_pos += 80; //5*(aplWeaponTotalTime[p->curr_weapon][snum]- (*kb)); //D
					weapon_xoffset+=80; //80*(*kb-aplWeaponTotalTime[p->curr_weapon][snum]);
                    myospal(weapon_xoffset+168-(p->look_ang>>1),looking_arc+260-gun_pos,
                        2518,gs,o,pal);
				}
				else
				{
					...
					// up and left
                    gun_pos += 10*( aplWeaponReload[p->curr_weapon][snum]- (*kb));
					  //5*(aplWeaponReload[p->curr_weapon][snum]- (*kb)); //U
					weapon_xoffset+=10*( aplWeaponReload[p->curr_weapon][snum]- (*kb));
...
#ifdef WW2WEPDISPLAYPISTOL
#ifdef WW2
//#define WEAPON2_RELOAD_TIME 50
                else if((*kb) < (aplWeaponReload[PISTOL_WEAPON][snum] - 12))
                {
...
#define WEAPON2_RELOAD_TIME 50
                ...
                else if((*kb) < 44)
                {
                    ...
                    myospal(194-(p->look_ang>>1),looking_arc+235-gun_pos,FIRSTGUN+5,gs,o,pal);
#else // APPVER_DN3DREV < AV_DR_NAM10
                else if((*kb) < 23)
                {
                    myospal(184-(p->look_ang>>1),looking_arc+235-gun_pos,FIRSTGUN+8,gs,o,pal);
                    myospal(224-(p->look_ang>>1),looking_arc+210-gun_pos,FIRSTGUN+5,gs,o,pal);
                ...
                else if((*kb) < 25)
                {
                    ...
                    myospal(194-(p->look_ang>>1),looking_arc+235-gun_pos,FIRSTGUN+5,gs,o,pal);
#endif // APPVER_DN3DREV >= AV_DR_NAM10
#endif					
//sprintf(g_szBuf,"CP:%s %d",__FILE__,__LINE__);
//AddLog(g_szBuf);
            }
        ...
        case HANDBOMB_WEAPON:
        {
            ...
            if((*kb))
            {
				...
				{
					// up and left
                    gun_pos += 10*((*kb)-aplWeaponFireDelay[HANDBOMB_WEAPON][snum]); //U
                ...
                else if((*kb) < aplWeaponTotalTime[HANDBOMB_WEAPON][snum])
				{
                    ...
                    gun_pos -= 12*((*kb)-aplWeaponFireDelay[HANDBOMB_WEAPON][snum]);  //D
					// move left
					weapon_xoffset+=90-(5*(aplWeaponTotalTime[HANDBOMB_WEAPON][snum]-(*kb)));
				}

//sprintf(g_szBuf,"HB: %d pos=%ld xoffset=%ld",*kb, gun_pos, weapon_xoffset);
//AddLog(g_szBuf);
#else
                if((*kb) < 7)
                    gun_pos -= 10*(*kb);        //D
            ...
            if((*kb))
            {
				if((*kb)<aplWeaponTotalTime[p->curr_weapon][snum] )
				{
					...
					if(p->ammo_amount[p->curr_weapon] & 1)
					{
						...
						myospal(weapon_xoffset+268-(p->look_ang>>1),looking_arc+238-gun_pos,DEVISTATOR+i,-32,o,pal);
					}
					else
					{
						myospal( weapon_xoffset+30-(p->look_ang>>1),looking_arc+240-gun_pos,DEVISTATOR+i,-32,o|4,pal);
...
#ifdef WW2WEPDISPLAY
        case SHRINKER_WEAPON:	// grenade launcher in WW2
            weapon_xoffset += 28;
            looking_arc += 18;
            if(sprite[p->i].pal == 1)
                pal = 1;
            ...
            if((*kb) == 0)
            {
//sprintf(g_szBuf,"Launcher display: kb=%d currw ammo=%d",*kb, ps[snum].ammo_amount[p->curr_weapon]);
//AddLog(g_szBuf);
//sprintf(g_szBuf," : cw ammo=%d", ps[snum].ammo_amount[cw]);
//AddLog(g_szBuf);
				// the 'at rest' display
...
//AddLog("At Rest");					
            ...
            {
//sprintf(g_szBuf,"Launcher display: kb=%d",*kb);
//AddLog(g_szBuf);
				// the 'active' display.
                if(sprite[p->i].pal != 1)
                {
                ...
                if(*kb < aplWeaponTotalTime[p->curr_weapon][snum])
                {
					...
					else
					{
						...
						gun_pos-=10*(aplWeaponTotalTime[p->curr_weapon][snum]-(*kb));
					}
                }
				// else we are in 'reload time'
                else if((*kb) <
			...
			break;
        case GROW_WEAPON:	// sniper rifle in WW2
            weapon_xoffset += 28;
            looking_arc += 18;
            if(sprite[p->i].pal == 1)
                pal = 1;
            ...
            {
//sprintf(g_szBuf,"sniper display: kb=%d",*kb);
//AddLog(g_szBuf);
                if(sprite[p->i].pal != 1)
                {
                    weapon_xoffset += rand()&3;
                ...
                if(*kb < aplWeaponTotalTime[p->curr_weapon][snum])
                {
					...
					else
					{
						...
						// lower weapon to reload cartridge (not clip)
						gun_pos-=15*(aplWeaponTotalTime[p->curr_weapon][snum]-(*kb));
					}
                }
				// else we are in 'reload time'
                else if((*kb) <
				...
				{
					// down 
                    gun_pos -= 5*((*kb)-aplWeaponTotalTime[p->curr_weapon][snum]); //D
				}
				else
				{
					// up
...
#ifdef EDUKE
    loc.bits |=   (ud.auto_run | BUTTON(gamefunc_Run))<<5;
#else
...
//            p->bobcounter += 32;
    ...
    */
    ...
    if ( p->fist_incs ||
         ...
         p->access_incs > 0 ||
         p->knee_incs > 0 ||
#if (defined WW2) || (APPVER_DN3DREV < AV_DR_WW2GI10)
//#ifdef WW2
...
#if (!defined WW2) || (APPVER_DN3DREV < AV_DR_EDK20021)
//#ifndef WW2
		// sb_snum&2 = fire
...
//				(*kb) = aplWeaponFireDelay[p->curr_weapon][snum]+1;	// animate, but don't shoot...
				(*kb) = aplWeaponTotalTime[p->curr_weapon][snum]+1;	// animate, but don't shoot...
...
//AddLog(g_szBuf);
    ...
    else if((*kb))
    {
		// already firing...
#ifdef WW2WEAPON
//sprintf(g_szBuf,"%s %ld, kb=%d %ld",__FILE__,__LINE__,*kb,aplWeaponWorksLike[p->curr_weapon][snum]);
//AddLog(g_szBuf);
...
//AddLog("SpawnTime");					
                ...
                if(*kb == aplWeaponFireDelay[p->curr_weapon][snum])
                {
//sprintf(g_szBuf,"FireDelayTime:%s %ld, kb=%d %ld",__FILE__,__LINE__,*kb,aplWeaponWorksLike[p->curr_weapon][snum]);
//AddLog(g_szBuf);
...
//							break;
				...
				{
//sprintf(g_szBuf,"end of TotalTime(%ld) %s %ld, kb=%d %ld",
//		aplWeaponTotalTime[p->curr_weapon][snum],
...
//AddLog(g_szBuf);
					  ...
					  )
					{
						// reload in progress...
						int i;
//sprintf(g_szBuf,"C %s %ld, kb=%d",__FILE__,__LINE__,*kb);
//AddLog(g_szBuf);
//sprintf(g_szBuf,"ammo=%d clip=%ld mod=%ld",p->ammo_amount[p->curr_weapon],
//		aplWeaponClip[p->curr_weapon][snum],
//		(p->ammo_amount[p->curr_weapon]%(aplWeaponClip[p->curr_weapon][snum]))
//		 );
//AddLog(g_szBuf);
					...
					{
//sprintf(g_szBuf,"NC %s %ld, kb=%d",__FILE__,__LINE__,*kb);
//AddLog(g_szBuf);
...
//sprintf(g_szBuf,"NA %s %ld, kb=%d",__FILE__,__LINE__,*kb);
//AddLog(g_szBuf);
							*kb=0;
						}
					}
...
//if( (*kb) != 50)
//{
//	FILE * fp=fopen("not50.con","w");
//	fprintf(fp,"aplWeaponReload[%d][snum]=%d (%d)\n",PISTOL_WEAPON,(short)aplWeaponReload[PISTOL_WEAPON][snum], p->curr_weapon);
//	DumpGameDefs(fp);
//	fclose(fp);
//}
